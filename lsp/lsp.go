...
var builtinDocs = map[string]builtinDoc{
	...
	"debug.assert":            {"debug.assert(cond, msg?) -> value", "Throws if cond is falsy. Returns cond unchanged on success."},
	"debug.invariant":         {"debug.invariant(cond, msg?) -> value", "Stronger-named alias of assert for production-critical checks."},
	...
	"vault.get":               {"vault.get(key) -> string", "AES-256-GCM decrypt the named secret from .vault.json (key from MX_VAULT_KEY env)."},
	"vault.set":               {"vault.set(key, value) -> string", "Encrypt + persist a secret. Run interactively to bootstrap, then commit .vault.json."},
	"vault.list":               {"vault.list() -> array", "List the keys in the vault (values stay encrypted)."},
	"vault.delete":             {"vault.delete(key)", "Remove a secret from the vault."},
	"sh.run":                   {"sh.run(cmd, args?, opts?) -> number", "Run a command, return just the exit code. Throws on plumbing errors."},
	"sh.output":                {"sh.output(cmd, args?, opts?) -> string", "Run + return stdout; throws on non-zero exit (stderr in error)."},
	...
	"server_timing":            {"server_timing(metrics) -> string", "Build a Server-Timing header value. Number values are durations in ms ('db;dur=23'); string values are descriptions ('cache;desc=\"hit\"'). Browser devtools surface these under Network → Timing."},
	"csv":                      {"csv(items, opts?) -> response", "Render array of objects as text/csv. Column order follows first row's keys. opts.filename sets Content-Disposition: attachment so browsers download instead of rendering."},
	"ndjson":                   {"ndjson(items) -> response", "Newline-delimited JSON. Each item on its own line; Content-Type: application/x-ndjson. The de-facto streaming-JSON shape."},
	...
	"body_validate":            {"body_validate(request, schema) -> { ok, body? } | { ok, response? }", "Validate request.body against a JSON Schema. Returns { ok: true, body } on success or { ok: false, response } where response is a fully-formed problem+json 400 with errors + request.id baked in."},
	...
	"sql.upsert":               {"sql.upsert(db, table, row, conflict_keys) -> { rows_affected, last_insert_id }", "INSERT or UPDATE. Picks ON CONFLICT (sqlite/postgres) or ON DUPLICATE KEY UPDATE (mysql) from the connection driver. conflict_keys is the unique-constraint column list."},
	"sql.update":               {"sql.update(db, table, set, where) -> { rows_affected, last_insert_id }", "UPDATE table SET col = ? WHERE col = ?. Both 'set' and 'where' are objects (where is AND-joined). Empty where errors — refuses to update every row."},
...
var snippets = []snippet{
	...
	{Prefix: "test", Description: "inline test block", Body: "test \"${1:behaviour}\" {\n\tassert_eq(${2:actual}, ${3:expected})\n}\n"},
	{Prefix: "testfn", Description: "test_* function (legacy form)", Body: "fn test_${1:name}() {\n\tassert_eq(${2:actual}, ${3:expected})\n}\n"},
	{Prefix: "bench", Description: "inline benchmark block", Body: "bench \"${1:operation}\" {\n\t${0}\n}\n"},
	{Prefix: "benchfn", Description: "bench_* function (legacy form)", Body: "fn bench_${1:name}() {\n\t${0}\n}\n"},
