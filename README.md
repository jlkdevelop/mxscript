...
| **Database** | SQLite + Postgres + MySQL through one `sql` namespace. Object-driven CRUD: `sql.find` / `find_one` / `count` / `exists` / `insert` / `upsert` / `update` / `delete` — no hand-rolled SQL strings. Plus `sql.exec` / `query` / `transaction` / `migrate` for the rest. Full-text search via `search.*` (FTS5) |
| **API helpers** | `paginate(request)` + `page_response(items, p, total)` for the standard list shape. `body_validate(request, schema)` returns `{ ok, body }` or a fully-formed 400 problem+json in one call. `problem(status, title, detail?)` for RFC 7807 errors. `etag(value)` + `not_modified()` + `cache_control({...})` for cacheable detail endpoints. `request.id` + `X-Request-ID` echo for free distributed tracing. `csv(items)` / `ndjson(items)` for export endpoints. `save_upload(file, path)` for multipart. `api_key_auth(request, allowed)` for service-to-service |
...
| **AI** | 13 built-in providers + custom — OpenAI, Anthropic, Gemini, xAI Grok, Mistral, DeepSeek, Groq, OpenRouter, Together, Ollama, Perplexity, Fireworks, Cerebras, plus `provider: "custom"` with `base_url` for self-hosted vLLM / LiteLLM / AI Gateway. `ai.complete`, `ai.stream`, `ai.embed`, `ai.vision`, `ai.image` (DALL-E), `ai.transcribe` (Whisper) |
